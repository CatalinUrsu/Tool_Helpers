...
#region Fields
    ...
    static Vector2 _dropdownPanelSizeDelta = new Vector2(0, 80);
...
#region UI Commands
    ...
    public static void CreateDropDown(MenuCommand menuCommand)
    {
        ...
        DropdownCaption GetDropdownCaptions()
        {
            ...
            caption.SetPrivateField("_arrowRT", imgArrow.GetComponent<RectTransform>());
        ...
        {
            var panelGO = CreateUIElementRoot("Panel", _dropdownSize, typeof(Image), typeof(DropdownPanel), typeof(Canvas), typeof(GraphicRaycaster), typeof(CanvasGroup));
            var viewportGO = CreateUIElementRoot("Viewport", _dropdownSize, typeof(RectMask2D));
            var contentGO = CreateUIElementRoot("Content", _dropdownSize, typeof(VerticalLayoutGroup), typeof(ContentSizeFitter));
            var scrollbarGO = CreateUIElementRoot("Scrollbar", _dropdownPanelSizeDelta, typeof(Image), typeof(Scrollbar));
            var scrollbarHandleGO = CreateUIElementRoot("Handle", _dropdownPanelSizeDelta, typeof(Image));
            var contentRT = contentGO.GetComponent<RectTransform>();
            var vieportRT = viewportGO.GetComponent<RectTransform>();
            var scrollbar = scrollbarGO.GetComponent<Scrollbar>();

            SetParentAndAlign(panelGO, dropdownGO);
            SetParentAndAlign(viewportGO, panelGO);
            SetParentAndAlign(contentGO, viewportGO);
            SetParentAndAlign(scrollbarGO, panelGO);
            SetParentAndAlign(scrollbarHandleGO, scrollbarGO);
            ...
            void SetContent()
            {
                var contentLG = contentGO.GetComponent<VerticalLayoutGroup>();
                var contentSizeFitter = contentGO.GetComponent<ContentSizeFitter>();

                contentRT.anchorMin = new Vector2(0, 1);
                contentRT.anchorMax = Vector2.one;
                contentRT.pivot = new Vector2(0.5f, 1);
                contentRT.sizeDelta = _dropdownPanelSizeDelta;

                contentLG.padding = new RectOffset(2, 2, 2, 2);
                contentLG.spacing = 2;
                contentLG.childControlWidth = true;
            ...
            void SetViewport()
            {
                ...
                vieportRT.pivot = new Vector2(0, .5f);
            }
            ...
            void SetScrollbar()
            {
                ...
                scrollbar.targetGraphic = scrollbarHandleRT.GetComponent<Image>();
                scrollbar.handleRect = scrollbarHandleRT;
                scrollbar.direction = Scrollbar.Direction.BottomToTop;

                scrollbarRT.anchorMin = new Vector2(1, 0);
                scrollbarRT.anchorMax = Vector2.one;
                scrollbarRT.pivot = new Vector2(1, .5f);
                scrollbarRT.anchoredPosition = new Vector2(-2, 0);
                scrollbarRT.sizeDelta = new Vector2(10, -4);

                scrollbarImage.raycastTarget = false;
                scrollbarHandleRT.sizeDelta = Vector2.zero;

                SetImage(scrollbarGO, _uiResources.background, Color.white, Image.Type.Sliced);
                SetImage(scrollbarHandleGO, _uiResources.standard, Color.white, Image.Type.Sliced);
            }
            ...
            void SetScrollRect()
            {
                ...
                scrollRectRT.sizeDelta = _dropdownPanelSizeDelta;
        ...
        DropdownItem GetDropdownItemTemplate()
        {
            ...
            void SetToggle()
            {
                ...
                item.Toggle = item.GetComponent<Toggle>();
                item.Toggle.targetGraphic = imgBg.GetComponent<Image>();
                item.Toggle.graphic = imgCheckmark.GetComponent<Image>();
            }
